...
void prepare_for_next_period(struct task_struct *t)
{
	BUG_ON(!t);
        ...
        //unsigned long long depass = (long long) litmus_clock() - t->rt_param.task_params.exec_cost ; // On ajoute une variable qui calcule le depassement.

        /* if (depass > 0) {
		t->rt_param.task_params.mode = 0;
	} */
	...
	t->rt_param.job_params.lateness =(long long)litmus_clock() - (long long)t->rt_param.job_params.deadline;

        
        /*************************** Notre partie ************/
/*
         if ( t->rt_param.task_params.criticite == 1 && t->rt_param.task_params.mode == 0 ) 
              t->rt_param.task_params.exec_cost = 0;
         else if ( t->rt_param.task_params.criticite == 0 && t->rt_param.task_params.mode == 0 ) t->rt_param.task_params.exec_cost = t->rt_param.task_params.wcet_high;
