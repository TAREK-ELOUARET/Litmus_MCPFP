...
DEFINE_PER_CPU(pfp_domain_t, mcpfp_domains);

pfp_domain_t* mcpfp_doms[NR_CPUS];

#define local_pfp		(this_cpu_ptr(&mcpfp_domains))
#define remote_dom(cpu)		(&per_cpu(mcpfp_domains, cpu).domain)
#define remote_pfp(cpu)	(&per_cpu(mcpfp_domains, cpu))
#define task_dom(task)		remote_dom(get_partition(task))
#define task_pfp(task)		remote_pfp(get_partition(task))


#ifdef CONFIG_LITMUS_LOCKING
DEFINE_PER_CPU(uint64_t,mcfmlp_timestamp);
#endif
...
static struct task_struct* mcpfp_schedule(struct task_struct * prev)
{
...
static void mcpfp_finish_switch(struct task_struct *prev)
{
	pfp_domain_t *to;
 ...
 */
static void mcpfp_task_new(struct task_struct * t, int on_rq, int is_scheduled)
{
...
static void mcpfp_task_wake_up(struct task_struct *task)
{
	unsigned long		flags;
...
static void mcpfp_task_block(struct task_struct *t)
{
...
static void mcpfp_task_exit(struct task_struct * t)
{
	unsigned long flags;
	pfp_domain_t* 	pfp = task_pfp(t);
	rt_domain_t*		dom;
...
fmlp_clock(void)
{
	return (lt_t) this_cpu_inc_return(mcfmlp_timestamp);
}

int mcpfp_fmlp_lock(struct litmus_lock* l)
{
...
int mcpfp_fmlp_unlock(struct litmus_lock* l)
{
...
int mcpfp_fmlp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct fmlp_semaphore *sem = fmlp_from_lock(l);
	unsigned long flags;
...
void mcpfp_fmlp_free(struct litmus_lock* lock)
{
	kfree(fmlp_from_lock(lock));
}

static struct litmus_lock_ops mcpfp_fmlp_lock_ops = {
	.close  = mcpfp_fmlp_close,
	.lock   = mcpfp_fmlp_lock,
	.unlock = mcpfp_fmlp_unlock,
	.deallocate = mcpfp_fmlp_free,
};
...
static struct litmus_lock* pfp_new_fmlp(void)
{
	...
	sem->litmus_lock.ops = &mcpfp_fmlp_lock_ops;
...
int mcpfp_mpcp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
...
int mcpfp_mpcp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current, *next = NULL;
	struct mpcp_semaphore *sem = mpcp_from_lock(l);
	unsigned long flags;
...
int mcpfp_mpcp_open(struct litmus_lock* l, void* config)
{
	struct task_struct *t = current;
	int cpu, local_cpu;
	struct mpcp_semaphore *sem = mpcp_from_lock(l);
...
int mcpfp_mpcp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct mpcp_semaphore *sem = mpcp_from_lock(l);
	unsigned long flags;
...
void mcpfp_mpcp_free(struct litmus_lock* lock)
{
	kfree(mpcp_from_lock(lock));
}

static struct litmus_lock_ops mcpfp_mpcp_lock_ops = {
	.close  = mcpfp_mpcp_close,
	.lock   = mcpfp_mpcp_lock,
	.open	= mcpfp_mpcp_open,
	.unlock = mcpfp_mpcp_unlock,
	.deallocate = mcpfp_mpcp_free,
};
...
static struct litmus_lock* pfp_new_mpcp(int vspin)
{
	...
	sem->owner   = NULL;
	init_waitqueue_head(&sem->wait);
	sem->litmus_lock.ops = &mcpfp_mpcp_lock_ops;
...
int mcpfp_pcp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
...
int mcpfp_pcp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct pcp_semaphore *sem = pcp_from_lock(l);
...
int mcpfp_pcp_open(struct litmus_lock* l, void* __user config)
{
	struct task_struct *t = current;
	struct pcp_semaphore *sem = pcp_from_lock(l);
...
int mcpfp_pcp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct pcp_semaphore *sem = pcp_from_lock(l);
...
void mcpfp_pcp_free(struct litmus_lock* lock)
{
	kfree(pcp_from_lock(lock));
}


static struct litmus_lock_ops mcpfp_pcp_lock_ops = {
	.close  = mcpfp_pcp_close,
	.lock   = mcpfp_pcp_lock,
	.open	= mcpfp_pcp_open,
	.unlock = mcpfp_pcp_unlock,
	.deallocate = mcpfp_pcp_free,
};
...
static struct litmus_lock* pfp_new_pcp(int on_cpu)
{
	...
	sem->litmus_lock.ops = &mcpfp_pcp_lock_ops;
...
int mcpfp_dpcp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
...
int mcpfp_dpcp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int err = 0;
...
int mcpfp_dpcp_open(struct litmus_lock* l, void* __user config)
{
	struct task_struct *t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int cpu, eprio;
...
int mcpfp_dpcp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct dpcp_semaphore *sem = dpcp_from_lock(l);
	int owner = 0;
...
void mcpfp_dpcp_free(struct litmus_lock* lock)
{
	kfree(dpcp_from_lock(lock));
}

static struct litmus_lock_ops mcpfp_dpcp_lock_ops = {
	.close  = mcpfp_dpcp_close,
	.lock   = mcpfp_dpcp_lock,
	.open	= mcpfp_dpcp_open,
	.unlock = mcpfp_dpcp_unlock,
	.deallocate = mcpfp_dpcp_free,
};
...
static struct litmus_lock* pfp_new_dpcp(int on_cpu)
{
	...
	sem->litmus_lock.ops = &mcpfp_dpcp_lock_ops;
...
int mcpfp_dflp_lock(struct litmus_lock* l)
{
	struct task_struct* t = current;
...
int mcpfp_dflp_unlock(struct litmus_lock* l)
{
	struct task_struct *t = current, *next;
	struct dflp_semaphore *sem = dflp_from_lock(l);
	int err = 0;
...
int mcpfp_dflp_open(struct litmus_lock* l, void* __user config)
{
	struct dflp_semaphore *sem = dflp_from_lock(l);
	int cpu;
...
int mcpfp_dflp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct dflp_semaphore *sem = dflp_from_lock(l);
	int owner = 0;
...
void mcpfp_dflp_free(struct litmus_lock* lock)
{
	kfree(dflp_from_lock(lock));
}

static struct litmus_lock_ops mcpfp_dflp_lock_ops = {
	.close  = mcpfp_dflp_close,
	.lock   = mcpfp_dflp_lock,
	.open	= mcpfp_dflp_open,
	.unlock = mcpfp_dflp_unlock,
	.deallocate = mcpfp_dflp_free,
};
...
static struct litmus_lock* pfp_new_dflp(int on_cpu)
{
	...
	sem->litmus_lock.ops = &mcpfp_dflp_lock_ops;
	sem->owner_cpu = NO_CPU;
	sem->owner   = NULL;
	sem->on_cpu  = on_cpu;
	init_waitqueue_head(&sem->wait);
...
static long mcpfp_allocate_lock(struct litmus_lock **lock, int type,
				 void* __user config)
{
	int err = -ENXIO, cpu;
...
static long mcpfp_admit_task(struct task_struct* tsk)
{
...
static long mcpfp_get_domain_proc_info(struct domain_proc_info **ret)
{
...
static long mcpfp_activate_plugin(void)
{
	...
	for_each_online_cpu(cpu) {
		...
		mcpfp_doms[cpu] = remote_pfp(cpu);
		per_cpu(mcfmlp_timestamp,cpu) = 0;
	}
...
static long mcpfp_deactivate_plugin(void)
{
...
static struct sched_plugin pfp_plugin __cacheline_aligned_in_smp = {
	.plugin_name		= "MCPFP",
	.task_new		= mcpfp_task_new,
	.complete_job		= complete_job,
	.task_exit		= mcpfp_task_exit,
	.schedule		= mcpfp_schedule,
	.task_wake_up		= mcpfp_task_wake_up,
	.task_block		= mcpfp_task_block,
	.admit_task		= mcpfp_admit_task,
	.activate_plugin	= mcpfp_activate_plugin,
	.deactivate_plugin	= mcpfp_deactivate_plugin,
	.get_domain_proc_info	= mcpfp_get_domain_proc_info,
#ifdef CONFIG_LITMUS_LOCKING
	.allocate_lock		= mcpfp_allocate_lock,
	.finish_switch		= mcpfp_finish_switch,
#endif
};


static int __init init_mcpfp(void)
{
	int i;
