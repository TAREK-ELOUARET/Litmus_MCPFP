...
#define OPTSTR "p:c:wlveo:F:s:m:q:r:X:L:Q:M:iRu:Bhd:C:"
int main(int argc, char** argv)
{
	...
	lt_t phase;
        ...
        // int mode = 1; /////////// Declaration de variable mode d'une tache ************ ///// 


	double wcet1_ms, wcet2_ms, period_ms, underrun_ms = 0;
	double offset_ms = 0, deadline_ms = 0;
	unsigned int priority = LITMUS_NO_PRIORITY;
	int migrate = 0;
	int cluster = 0;
        static int criticite = 0; /////////// Declaration de variable criticité d'une tache ************ /////  
	...
	while ((opt = getopt(argc, argv, OPTSTR)) != -1) {
		...
		case 'F':
			...
			break;
                ...
                case 'M': 
                         criticite =atoi(optarg); //////////// ********* PArtie faite par *********//////////////
                         break;
		case 'm':
	...
	wcet1_ms   = atof(argv[optind + 0]);
        wcet2_ms   = atof(argv[optind + 1]);
	period_ms = atof(argv[optind + 2]);

	wcet1   = ms2ns(wcet1_ms);
        wcet2   = ms2ns(wcet2_ms);
	...
	if (!file && (wcet1 > period || wcet2 > period) ) {
	...
	param.exec_cost = wcet1;
        param.period = period;
        param.phase  = phase;
	 // param.wcet_high = wcet2;
	...
	if (create_reservation) {
		...
		config.polling_params.budget = wcet1; 
	...
	if (file) {
		...
		do {
			...
			acet = (wcet1_ms - drand48() * underrun_ms) * 0.001;
                        acet2 = (wcet2_ms - drand48() * underrun_ms) * 0.001;
			if (acet < 0)
				acet = 0;
			acet *= scale;
			...
			if (verbose) {
                                printf("\ttarget exec. time: %6.2fms (%.2f%% of WCET)\n",acet2 * 1000,(acet2 * 1000 / wcet2_ms) * 100);
                                     }
                        if (itteration >= 4) {
                            if (criticite == 0) {
                            acet = acet2;
                            param.exec_cost = wcet2;}
                            else {
                            acet = 0;
                            param.exec_cost = 0; }}
		...
		} while (job(acet,start + duration,lock_od, cs_length * 0.001));
	}
